...
build.mjs               concatenates the above into dist/index.html
...
dist/index.html         built output (committed, ready to publish)
```

Load order is fixed and matters: data globals → `core.js` (defines `window.NB`) → `views.js` (consumes it, then calls `NB.boot()`).
...
Private advisor notes live in a separate `notes/` collection restricted to editors. A client cannot read them through the page or underneath it; the store itself refuses.
...
The next version is an ordinary web app:
