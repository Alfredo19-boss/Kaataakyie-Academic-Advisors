/* Katakyie Academic Advisor — views and interaction. */
(function (NB) {
"use strict";
...
function renderLogin() {
  ...
  var org = esc(S.cfg.orgName || "Katakyie Academic Advisor");
  host.innerHTML =
    '<div class="brandside">' +
      '<div style="display:flex;align-items:center;gap:11px">' +
        '<svg class="logomark" viewBox="0 0 64 64" aria-hidden="true"><rect width="64" height="64" rx="15" fill="#0C0A06"></rect>' +
        '<path d="M32 13 51 32" fill="none" stroke="var(--brand)" stroke-width="6" stroke-linecap="round"/><path d="M32 13 13 32" fill="none" stroke="var(--brand)" stroke-width="6" stroke-linecap="round"/><path d="M32 31 51 50" fill="none" stroke="#9A7C2E" stroke-width="6" stroke-linecap="round"/><path d="M32 31 13 50" fill="none" stroke="#9A7C2E" stroke-width="6" stroke-linecap="round"/>' + "</svg>" +
...
function render() {
  ...
  $("#brandName").textContent = S.cfg.orgName || "Katakyie Academic Advisor";
...
function viewClients(v) {
  ...
  var rows = list.map(function (c) {
    ...
    return '<tr class="clickable" data-act="openclient" data-v="' + esc(c.id) + '">' +
      ...
      '<td style="text-align:right"><button class="btn sm" data-act="sharelink" data-v="' + esc(c.id) + '">Send link</button></td>' +
      "</tr>";
  }).join("");
  ...
  v.insertAdjacentHTML("beforeend",
    ...
    "<thead><tr><th>Client</th><th>Intake</th><th>Stage</th><th>Completion</th><th>Next deadline</th><th>Schedule</th><th>Status</th><th>Updated</th><th></th></tr></thead>" +
    "<tbody>" + (rows || '<tr><td colspan="9"><div class="empty"><p>No client matches that search.</p></div></td></tr>') + "</tbody></table></div></div></div>");
}
...
function viewClient(v) {
  ...
  if (isAdvisor()) {
    ...
    $("#topactions").innerHTML =
      ...
      '<button class="btn pri" data-act="sharelink" data-v="' + esc(c.id) + '">Send client link</button>' +
  ...
  return '<div class="card-h no-print"><div><h3>Status report</h3>' +
    ...
    '<div class="card-b rep">' +
      '<div class="printonly" style="margin-bottom:18px;border-bottom:2px solid var(--brand);padding-bottom:10px">' +
        '<div style="font-family:var(--serif);font-size:22px;font-weight:600">' + esc(S.cfg.orgName || "Katakyie Academic Advisor") + "</div>" +
      ...
      "Prepared by " + esc(S.cfg.orgName || "Katakyie Academic Advisor") + " on " + fmtDate(NB.today()) +
...
function viewSettings(v) {
  ...
  v.insertAdjacentHTML("beforeend",
    ...
    '<div class="card"><div class="card-h"><div><h3>Practice details</h3>' +
      ...
      '<div class="field"><label for="setSite">Public site address</label><input class="inp mono" id="setSite" value="' + esc(S.cfg.siteUrl || "") + '" placeholder="https://yourname.github.io/your-repo"></div>' +
      '<div class="field"><label for="setMail">Your email, for the button on client links</label><input class="inp" id="setMail" value="' + esc(S.cfg.advisorEmail || "") + '" placeholder="you@example.com"></div>' +
...
/* ---------------------------------- share a client link ---------------------------------- */
/* Packs one client's file into a URL fragment for the read-only portal on the public site.
   Private notes, fees and referee contact details are deliberately left out. */
...
function snapshotOf(c, note) {
  var tpl = NB.tplOf(c);
  var custom = (c.template && c.template !== "default") ? tpl.map(function (t) {
    return { id: t.id, s: t.s, o: t.o, t: t.t, h: t.h };
  }) : null;
  return {
    v: 1, at: NB.today(), org: S.cfg.orgName || "", adv: S.cfg.advisorEmail || "",
    n: c.name || "", f: c.field || "", t: c.targetTerm || "", st: c.status || "",
    msg: note || "",
    ts: c.tasks || {}, du: c.due || {}, dc: c.docs || {},
    tpl: custom,
    sc: (c.schools || []).map(function (r) {
      return { n: r.name, p: r.program || "", lo: r.loc || "", dl: r.deadline || "", s: r.status || "",
               tu: num(r.tuition), fe: num(r.fees), li: num(r.living), aw: num(r.award) };
    })
  };
}
function buildShareLink(c, note) {
  var base = String(S.cfg.siteUrl || "").replace(/\/+$/, "");
  if (!base) return Promise.reject({ nobase: true });
  var json = JSON.stringify(snapshotOf(c, note));
  var bytes = new TextEncoder().encode(json);
  return deflate(bytes)
    .then(function (z) { return "z" + bytesToB64url(z); })
    .catch(function () { return "u" + bytesToB64url(bytes); })
    .then(function (frag) { return base + "/portal/#" + frag; });
}
function shareModal(c) {
  if (!String(S.cfg.siteUrl || "").trim()) {
    modal("Set your site address first",
      "<p>A client link points at the portal page on your public site, so the platform needs to know where that site lives.</p>" +
      '<p class="muted" style="font-size:13px">Open Settings and put your site address in \u2014 something like <span class="mono">https://yourname.github.io/your-repo</span> \u2014 then come back.</p>',
      "Open Settings", function () { S.view = "settings"; render(); return true; });
    return;
  }
  modal("Send " + (c.name || "this client") + " their link",
    '<div class="field"><label for="shNote">A line at the top of their page (optional)</label>' +
    '<textarea class="inp" id="shNote" style="min-height:66px" placeholder="Speaking retake is booked for 6 October \u2014 everything else is on track."></textarea></div>' +
    '<div class="field"><label for="shUrl">Their link</label>' +
    '<textarea class="inp mono" id="shUrl" style="min-height:96px;font-size:11.5px" readonly>Building\u2026</textarea></div>' +
    '<div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-act="shcopy">Copy link</button>' +
    '<a class="btn" id="shOpen" target="_blank" rel="noopener" href="#">Open it yourself</a></div>' +
    '<p class="muted" style="font-size:12.5px;line-height:1.5">The whole file travels inside the link, so there is nothing for them to sign into. ' +
    "It shows their progress, schools, documents and dates \u2014 never your private notes or anything about fees. " +
    "It is a snapshot: send a fresh link when you want them to see the current picture, and treat the link as private, " +
    "because anyone holding it can read that page.</p>",
    "Done", function () { return true; });

  var out = $("#shUrl"), open = $("#shOpen"), note = $("#shNote");
  function refresh() {
    buildShareLink(c, note ? note.value.trim() : "").then(function (url) {
      if (!$("#shUrl")) return;
      $("#shUrl").value = url;
      if ($("#shOpen")) $("#shOpen").href = url;
    }).catch(function () { if ($("#shUrl")) $("#shUrl").value = "Could not build the link."; });
  }
  refresh();
  if (note) note.addEventListener("input", function () { clearTimeout(note._t); note._t = setTimeout(refresh, 250); });
}
...
document.addEventListener("click", function (e) {
  ...
  if (a === "savecfg") {
    S.cfg.orgName = $("#setName").value.trim() || "Katakyie Academic Advisor";
    S.cfg.advisorCode = $("#setCode").value.trim();
    S.cfg.welcome = $("#setWelcome").value.trim();
    S.cfg.siteUrl = $("#setSite").value.trim().replace(/\/+$/, "");
    S.cfg.advisorEmail = $("#setMail").value.trim();
  ...
  if (a === "sharelink") { var cs3 = byId(v) || meClient(); if (cs3) shareModal(cs3); return; }
  if (a === "shcopy") {
    var box = $("#shUrl"); if (!box) return;
    box.focus(); box.select();
    var ok = false;
    try { ok = document.execCommand("copy"); } catch (e) {}
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(box.value).then(function () { toast("Link copied"); }, function () {
        toast(ok ? "Link copied" : "Select the link and copy it manually.");
      });
    } else { toast(ok ? "Link copied" : "Select the link and copy it manually."); }
    return;
  }
